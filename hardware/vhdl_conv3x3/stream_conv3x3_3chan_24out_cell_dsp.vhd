...
-- DSP-aware 24-output extension of stream_conv3x3_3chan_16out_cell_dsp.vhd.
--
-- This is NOT a full U-Net or full Conv2d layer, and it is NOT a new
-- architecture -- it is structurally IDENTICAL to
-- stream_conv3x3_3chan_16out_cell_dsp.vhd (same window-sharing structure,
-- same conv3x3_dot_pipelined_dsp DSP-steered multiply mapping), just
-- mechanically extended from 16 to 24 output kernels (kernels 0-23 of
-- enc1.block.0.weight, from first_layer_kernels0_to23_pkg).
--
-- Motivation: real synthesized DSP-aware data points already exist for 8
-- and 16 output channels on the Artix-7 200T
-- (hardware/vhdl_conv3x3/first_layer_16out_dsp_200t_summary.md):
--   8-output:  668 LUTs, 410 regs, 203/740 DSPs, WNS +3.848 ns, 0 warnings
--   16-output: 929 LUTs, 418 regs, 405/740 DSPs, WNS +3.848 ns, 0 warnings
-- This design is a third real scaling point (24 outputs) between the
-- verified 16-output design and the estimated 32-output projection. Not
-- board-tested, not a measured-speedup or measured-power claim, and not
-- all 32 first-layer output channels.
--
-- ---- Architecture (window reuse, unchanged from the 8/16-output DSP variants) --
-- Three window3x3_stream instances (one PER INPUT CHANNEL, not per kernel):
-- all twenty-four output kernels read the SAME c0_p0..c0_p8 / c1_p* / c2_p*
-- window signals, just with different weight constants.
--
--   window3x3_stream x3               sliding 3x3 window buffers, shared by all kernels
--   conv3x3_dot_pipelined_dsp x72     24 kernels x 3 input channels, zero bias each,
--                                     DSP-steered multiplies
--   final_sum x24 (1 register each)   y{k}_r <= bias_k + yk_c0 + yk_c1 + yk_c2
--
...
-- valid to gate all twenty-four final summation stages.
--
-- ---- Latency ---------------------------------------------------------------
-- 4 clock cycles from triggering pixel to visible output -- IDENTICAL to
-- stream_conv3x3_3chan_16out_cell_dsp.vhd, since conv3x3_dot_pipelined_dsp has
-- the same 3-stage pipeline latency regardless of kernel count.
--
-- ---- Weight source -----------------------------------------------------
-- Kernels 0-23 (INT8 weights, symmetric per-tensor quantization) are taken
-- directly from hardware/vhdl_conv3x3/first_layer_kernels0_to23_pkg.vhd,
-- generated from enc1.block.0.weight of the trained Alea-tuned checkpoint
-- by scripts/export_first_layer_multi_kernel_vhdl_vectors.py 0-23.
-- Bias is 0 for all twenty-four kernels (Conv2d has no direct bias;
-- BatchNorm is NOT folded in here).
--
-- ---- Arithmetic width ---------------------------------------------------
-- INT8 x INT8 -> INT32 per dot product, identical to the 1/4/8/16-output
-- cells. 72 DSP-mappable multiply groups (24 kernels x 3 channels x
-- 3x3=9 taps each = 648 multiplies total) run in parallel.
--
-- ---- Scope ----------------------------------------------------------------
-- This is a 24-output first-layer convolution HARDWARE PROTOTYPE, not full
...
-- the existing verified 16-output DSP-aware design; it is not a new
-- architecture.

library IEEE;
use IEEE.std_logic_1164.all;
use IEEE.numeric_std.all;
use work.first_layer_kernels0_to23_pkg.all;

entity stream_conv3x3_3chan_24out_cell_dsp is
    generic (
    ...
    port (
        ...
        -- Output: twenty-four INT32 results (one per kernel) per valid window
        -- triplet, 4-cycle latency -- identical timing to the 1/4/8/16-output cells.
...
end entity stream_conv3x3_3chan_24out_cell_dsp;

architecture rtl of stream_conv3x3_3chan_24out_cell_dsp is
    ...
    -- kernel's bias once).
    -- ----------------------------------------------------------------
    signal y0_r, y1_r, y2_r, y3_r, y4_r, y5_r, y6_r, y7_r, y8_r, y9_r, y10_r, y11_r, y12_r, y13_r, y14_r, y15_r, y16_r, y17_r, y18_r, y19_r, y20_r, y21_r, y22_r, y23_r : signed(31 downto 0) := (others => '0');
    signal valid_r                : std_logic            := '0';

begin

    -- ================================================================
    -- Window generators -- ONE PER INPUT CHANNEL, shared by all 24 kernels.
    -- ================================================================
    ...
    -- Same structure as the 8/16-output DSP-aware cells' final_sum,
    -- replicated 24x -- this is why the cell's overall latency (4 cycles)
    -- is unchanged from the 1/4/8/16-output cells.
    -- ================================================================
