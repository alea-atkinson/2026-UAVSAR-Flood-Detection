-- tb_stream_conv3x3_3chan_8out_cell.vhd
-- Self-checking testbench for stream_conv3x3_3chan_8out_cell.
--
-- Extends tb_stream_conv3x3_3chan_4out_cell.vhd's approach (checking all
-- outputs at every valid_out pulse from a single streaming pass) to eight
-- parallel kernel outputs. This is a hardware-prototype testbench, not a
-- sequential per-kernel weight-reload verification harness.
--
-- ---- Source of weights --------------------------------------------------
...
--              stream_conv3x3_3chan_8out_cell via first_layer_kernels0_to7_pkg
...
--
-- ---- What this validates -----------------------------------------------
-- This testbench confirms that stream_conv3x3_3chan_8out_cell produces, in
-- ONE streaming pass, the same 9x8=72 INT32 values that the Python
-- golden-vector script produces for kernels 0-7. It does NOT validate full
-- quantized U-Net inference (BatchNorm is not applied), does NOT cover the
-- remaining 24 of 32 first-layer output channels, and has NOT been run on
...
entity tb_stream_conv3x3_3chan_8out_cell is
end entity tb_stream_conv3x3_3chan_8out_cell;

architecture sim of tb_stream_conv3x3_3chan_8out_cell is
...
begin
    ...
    -- ----------------------------------------------------------------
    -- DUT: stream_conv3x3_3chan_8out_cell
    -- ----------------------------------------------------------------
    dut : entity work.stream_conv3x3_3chan_8out_cell
    ...
    begin
        ...
        report "=== All stream_conv3x3_3chan_8out_cell tests PASSED ===" &
        ...
        report "    NOTE: 8-output first-layer convolution PROTOTYPE only." &
