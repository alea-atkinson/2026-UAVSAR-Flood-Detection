...
-- DSP-aware variant of stream_conv3x3_3chan_8out_cell.vhd.
--
-- This is NOT a full U-Net or full Conv2d layer, and it is NOT a new
-- architecture -- it is structurally IDENTICAL to
-- stream_conv3x3_3chan_8out_cell.vhd (same window-sharing structure, same
-- 8 kernels 0-7, same package of real trained-model weights), except every
-- conv3x3_dot_pipelined instance is replaced with conv3x3_dot_pipelined_dsp,
-- which adds a `use_dsp` synthesis attribute to each of its nine multiply
-- results to steer Vivado toward DSP48E1 slices instead of LUT fabric.
--
-- Motivation: the LUT-only 8-output baseline synthesizes to 45.07% LUT
-- utilization at 0% DSP utilization on xc7a35tcpg236-1. This variant tests
-- whether DSP-aware synthesis reduces LUT pressure before considering a
-- 16- or 32-output scale-up. See vivado_8out_dsp_synthesis_summary.md for
-- the resulting resource/timing/power comparison.
--
-- ---- Architecture (window reuse, unchanged from the LUT-only baseline) ---
-- Three window3x3_stream instances (one PER INPUT CHANNEL, not per kernel):
-- all eight output kernels read the SAME c0_p0..c0_p8 / c1_p* / c2_p* window
-- signals, just with different weight constants.
--
--   window3x3_stream ×3              sliding 3x3 window buffers, shared by all kernels
--   conv3x3_dot_pipelined_dsp ×24    8 kernels x 3 input channels, zero bias each,
...
-- ---- Latency ---------------------------------------------------------------
-- 4 clock cycles from triggering pixel to visible output -- IDENTICAL to
-- stream_conv3x3_3chan_8out_cell.vhd, since conv3x3_dot_pipelined_dsp has the
-- same 3-stage pipeline latency as conv3x3_dot_pipelined; only the multiply
-- mapping (LUT vs. DSP48E1) differs, not the cycle-accurate behavior.
--
-- ---- Weight source -----------------------------------------------------
-- Kernels 0-7 (INT8 weights, symmetric per-tensor quantization) are taken
-- directly from hardware/vhdl_conv3x3/first_layer_kernels0_to7_pkg.vhd
-- (same package as the LUT-only baseline; not duplicated for this variant).
-- Bias is 0 for all eight kernels (Conv2d has no direct bias; BatchNorm is
-- NOT folded in here).
--
-- ---- Scope ----------------------------------------------------------------
-- This is a synthesis-level DSP-mapping experiment for an 8-output
-- first-layer convolution HARDWARE PROTOTYPE, not full U-Net FPGA inference,
-- not board-tested, and not a measured speedup claim. It demonstrates
-- whether requesting DSP48E1 mapping changes resource/timing/power
-- tradeoffs relative to the LUT-only 8-output baseline.
...
entity stream_conv3x3_3chan_8out_cell_dsp is
    generic (
...
end entity stream_conv3x3_3chan_8out_cell_dsp;

architecture rtl of stream_conv3x3_3chan_8out_cell_dsp is
