-- stream_conv3x3_3chan_kernel3_bn_relu_real_tile_q20.vhd
-- REAL-TILE Q.20-format variant of stream_conv3x3_3chan_kernel3_bn_relu_pipelined.vhd (structurally, if it existed for this kernel).
--
-- This is NOT a new convolution architecture and NOT a new numerical
-- scheme. It is structurally IDENTICAL to
-- stream_conv3x3_3chan_kernel3_bn_relu_pipelined.vhd (structurally, if it existed for this kernel): the SAME existing,
-- unmodified stream_conv3x3_3chan_cell sub-component (kernel-3 folded INT8
...
-- constants from: first_conv_bn_relu_kernel3_real_tile_q20_pkg (REAL-TILE
...
--
-- ---- Scope ----------------------------------------------------------------
-- Kernel 3 ONLY (of the complete first Conv2d layer's 32 output channels; verified TOGETHER with all other 31 kernels by the combined all-32-kernel testbench). REAL UAVSAR-tile-derived
...
entity stream_conv3x3_3chan_kernel3_bn_relu_real_tile_q20 is
    generic (
...
end entity stream_conv3x3_3chan_kernel3_bn_relu_real_tile_q20;

architecture rtl of stream_conv3x3_3chan_kernel3_bn_relu_real_tile_q20 is
...
begin
    ...
    -- fed kernel 3's FOLDED INT8 weights (IDENTICAL values to the
    ...
    raw_conv : entity work.stream_conv3x3_3chan_cell
        ...
        port map (
            ...
            c0_w0 => to_signed(K3_RT_FOLDED_CH0_W(0), 8),
            c0_w1 => to_signed(K3_RT_FOLDED_CH0_W(1), 8),
            c0_w2 => to_signed(K3_RT_FOLDED_CH0_W(2), 8),
            c0_w3 => to_signed(K3_RT_FOLDED_CH0_W(3), 8),
            c0_w4 => to_signed(K3_RT_FOLDED_CH0_W(4), 8),
            c0_w5 => to_signed(K3_RT_FOLDED_CH0_W(5), 8),
            c0_w6 => to_signed(K3_RT_FOLDED_CH0_W(6), 8),
            c0_w7 => to_signed(K3_RT_FOLDED_CH0_W(7), 8),
            c0_w8 => to_signed(K3_RT_FOLDED_CH0_W(8), 8),
            c1_w0 => to_signed(K3_RT_FOLDED_CH1_W(0), 8),
            c1_w1 => to_signed(K3_RT_FOLDED_CH1_W(1), 8),
            c1_w2 => to_signed(K3_RT_FOLDED_CH1_W(2), 8),
            c1_w3 => to_signed(K3_RT_FOLDED_CH1_W(3), 8),
            c1_w4 => to_signed(K3_RT_FOLDED_CH1_W(4), 8),
            c1_w5 => to_signed(K3_RT_FOLDED_CH1_W(5), 8),
            c1_w6 => to_signed(K3_RT_FOLDED_CH1_W(6), 8),
            c1_w7 => to_signed(K3_RT_FOLDED_CH1_W(7), 8),
            c1_w8 => to_signed(K3_RT_FOLDED_CH1_W(8), 8),
            c2_w0 => to_signed(K3_RT_FOLDED_CH2_W(0), 8),
            c2_w1 => to_signed(K3_RT_FOLDED_CH2_W(1), 8),
            c2_w2 => to_signed(K3_RT_FOLDED_CH2_W(2), 8),
            c2_w3 => to_signed(K3_RT_FOLDED_CH2_W(3), 8),
            c2_w4 => to_signed(K3_RT_FOLDED_CH2_W(4), 8),
            c2_w5 => to_signed(K3_RT_FOLDED_CH2_W(5), 8),
            c2_w6 => to_signed(K3_RT_FOLDED_CH2_W(6), 8),
            c2_w7 => to_signed(K3_RT_FOLDED_CH2_W(7), 8),
            c2_w8 => to_signed(K3_RT_FOLDED_CH2_W(8), 8),
            bias      => ZERO32,
            valid_out => raw_valid,
            y         => raw_y
        );
    ...
    -- value, from first_conv_bn_relu_kernel3_real_tile_q20_pkg).
    -- ================================================================
