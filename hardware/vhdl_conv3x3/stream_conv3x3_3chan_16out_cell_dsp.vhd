...
-- DSP-aware 16-output extension of stream_conv3x3_3chan_8out_cell_dsp.vhd.
--
-- This is NOT a full U-Net or full Conv2d layer, and it is NOT a new
-- architecture -- it is structurally IDENTICAL to
-- stream_conv3x3_3chan_8out_cell_dsp.vhd (same window-sharing structure,
-- same conv3x3_dot_pipelined_dsp DSP-steered multiply mapping), just
-- mechanically extended from 8 to 16 output kernels (kernels 0-15 of
-- enc1.block.0.weight, from first_layer_kernels0_to15_pkg).
--
-- Motivation: the larger-FPGA-target comparison
-- (hardware/vhdl_conv3x3/larger_fpga_target_comparison_summary.md) showed
-- that the 8-output DSP-aware design's DSP overutilization and LUT-fallback
-- on the Artix-7 35T (90/90 DSPs, 6991 LUTs) disappears entirely on the
-- larger Artix-7 200T (203/740 DSPs, 668 LUTs, 0 warnings). This design
-- tests whether that headroom lets a MORE parallel (16-output) DSP-aware
-- first-layer design synthesize cleanly on the 200T -- a real synthesized
-- midpoint between the verified 8-output design and the estimated
-- 32-output scaling projection. Not board-tested, not a measured-speedup
-- or measured-power claim, and not all 32 first-layer output channels.
--
-- ---- Architecture (window reuse, unchanged from the 8-output DSP variant) --
-- Three window3x3_stream instances (one PER INPUT CHANNEL, not per kernel):
-- all sixteen output kernels read the SAME c0_p0..c0_p8 / c1_p* / c2_p*
-- window signals, just with different weight constants.
--
--   window3x3_stream ×3               sliding 3x3 window buffers, shared by all kernels
--   conv3x3_dot_pipelined_dsp ×48     16 kernels x 3 input channels, zero bias each,
--                                     DSP-steered multiplies
--   final_sum ×16 (1 register each)   y{k}_r <= bias_k + yk_c0 + yk_c1 + yk_c2
--
...
-- valid to gate all sixteen final summation stages.
--
-- ---- Latency ---------------------------------------------------------------
-- 4 clock cycles from triggering pixel to visible output -- IDENTICAL to
-- stream_conv3x3_3chan_8out_cell_dsp.vhd, since conv3x3_dot_pipelined_dsp has
-- the same 3-stage pipeline latency regardless of kernel count.
--
-- ---- Weight source -----------------------------------------------------
-- Kernels 0-15 (INT8 weights, symmetric per-tensor quantization) are taken
-- directly from hardware/vhdl_conv3x3/first_layer_kernels0_to15_pkg.vhd,
-- generated from enc1.block.0.weight of the trained Alea-tuned checkpoint
-- by scripts/export_first_layer_multi_kernel_vhdl_vectors.py 0-15.
-- Bias is 0 for all sixteen kernels (Conv2d has no direct bias; BatchNorm is
-- NOT folded in here).
--
-- ---- Arithmetic width ---------------------------------------------------
-- INT8 x INT8 -> INT32 per dot product, identical to the 1/4/8-output cells.
-- Forty-eight DSP-mappable multiply groups (16 kernels x 3 channels x 3x3=9
-- taps each = 432 multiplies total) run in parallel.
--
-- ---- Scope ----------------------------------------------------------------
-- This is a 16-output first-layer convolution HARDWARE PROTOTYPE, not full
-- U-Net FPGA inference, not board-tested, and does not include BatchNorm
-- folding, padding, ReLU, or a measured speedup/power figure. It extends the
-- existing verified 8-output DSP-aware design; it is not a new architecture.
...
entity stream_conv3x3_3chan_16out_cell_dsp is
    generic (
    ...
    port (
        ...
        -- Output: sixteen INT32 results (one per kernel) per valid window
        -- triplet, 4-cycle latency -- identical timing to the 1/4/8-output cells.
...
end entity stream_conv3x3_3chan_16out_cell_dsp;

architecture rtl of stream_conv3x3_3chan_16out_cell_dsp is
    ...
    -- All forty-eight dot products share identical valid_in timing, so
    ...
    -- kernel's bias once).
    -- ----------------------------------------------------------------
    signal y0_r, y1_r, y2_r, y3_r, y4_r, y5_r, y6_r, y7_r, y8_r, y9_r, y10_r, y11_r, y12_r, y13_r, y14_r, y15_r : signed(31 downto 0) := (others => '0');
    signal valid_r                : std_logic            := '0';

begin

    -- ================================================================
    -- Window generators -- ONE PER INPUT CHANNEL, shared by all 16 kernels.
    -- ================================================================
    ...
    -- Same structure as the 8-output DSP-aware cell's final_sum,
    -- replicated 16x -- this is why the cell's overall latency (4 cycles)
    -- is unchanged from the 1/4/8-output cells.
    -- ================================================================
