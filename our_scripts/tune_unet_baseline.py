...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-name", default="hyperparam_tuning_run")
...
def objective(trial, train_dataset, val_dataset, args):
...
def main() -> None:
    ...
    #print best parameters and best iou
    ...
    #set equal to best params and retrain model on train and val sets combined, then evaluate on test set
    ...
    model = UNet(in_channels=3, out_channels=1, base_channels=best["base_channels"]).to(device)
    loss_fn = nn.BCEWithLogitsLoss()
    optimizer = torch.optim.Adam(
    model.parameters(),
