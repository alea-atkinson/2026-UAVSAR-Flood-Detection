#!/usr/bin/env python3
"""
generate_first_layer_16out_folded_arithmetic_core_pkg.py

Generates hardware/vhdl_conv3x3/first_layer_16out_folded_bn_relu_real_tile_q20_pkg.vhd,
the weight/scale/bias package for a NEW 16-output arithmetic-core artifact
(hardware/vhdl_conv3x3/first_layer_16out_folded_arithmetic_core.vhd) that
computes only output channels 0-15 of the first learned Conv2d+BatchNorm+
ReLU stage.

---- Why this is a SLICE, not a new derivation -------------------------------
This repo already has a consolidated, already-verified, all-32-kernel Q.20
package: hardware/vhdl_conv3x3/first_layer_32out_folded_bn_relu_real_tile_q20_pkg.vhd
...
anything from the checkpoint, the tile file, torch, or rasterio, and does
NOT re-parse the raw weight / per-kernel real-tile files either. It parses
ONLY the existing 32-kernel merged package, SLICES out kernels 0-15, and
cross-checks that slice against the existing, independently-generated
per-kernel real-tile Q.20 packages
(first_conv_bn_relu_kernel{0..15}_real_tile_q20_pkg.vhd) as a genuine
...
Tensor key : enc1.block.0.weight (output channels 0-15 of 32), folded with
...
---- Output -------------------------------------------------------------------
hardware/vhdl_conv3x3/first_layer_16out_folded_bn_relu_real_tile_q20_pkg.vhd
"""
...
OUT_PKG_PATH = HW_DIR / "first_layer_16out_folded_bn_relu_real_tile_q20_pkg.vhd"
OUT_PKG_NAME = "first_layer_16out_folded_bn_relu_real_tile_q20_pkg"
...
def main() -> None:
    ...
    print("=" * 78)
    ch_weights_16 = {c: ch_weights_32[c][:NUM_KERNELS] for c in range(3)}
    scale_fx_16 = scale_fx_32[:NUM_KERNELS]
    bias_fx_16 = bias_fx_32[:NUM_KERNELS]
    ...
    for k in range(NUM_KERNELS):
        ...
        if rt_scale != scale_fx_16[k]:
            mismatches.append((k, "SCALE_FX"))
        rt_bias = parse_scalar_int(ktext, "BIAS_FX")
        if rt_bias != bias_fx_16[k]:
            mismatches.append((k, "BIAS_FX"))
    if mismatches:
        raise SystemExit(f"ERROR: 16-kernel slice mismatch against per-kernel "
    ...
    # ------------------------------------------------------------------
    # Write the sliced 16-kernel VHDL package
    # ------------------------------------------------------------------
    print("\n" + "=" * 78)
    ...
    for c in range(3):
        entries = ",\n        ".join(
            vhdl_kernel_array(ch_weights_16[c][k]) for k in range(NUM_KERNELS)
        )
        ch_lut_blocks.append(
            f"    constant CH{c}_WEIGHT_LUT : weight_lut_t := (\n        {entries}\n    );"
        )

    scale_fx_entries = ", ".join(str(v) for v in scale_fx_16)
    bias_fx_entries = ", ".join(str(v) for v in bias_fx_16)

    pkg_text = f"""\
-- {OUT_PKG_NAME}.vhd
-- 16-kernel (output channels 0-{NUM_KERNELS - 1} of 32) Q.20 real-tile folded
...
-- {NUM_KERNELS} Conv-BN-ReLU outputs in a single pipelined pass. It does NOT
