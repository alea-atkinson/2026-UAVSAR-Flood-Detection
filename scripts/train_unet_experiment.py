#!/usr/bin/env python3
"""Train an experimental SAR-only U-Net for flood/change segmentation.

This is an extended variant of train_unet_baseline.py that adds:
  - Selectable loss: bce, dice, bce_dice
  - Checkpoint selection by val_loss or val_dice (default: val_dice)
  - Optional train-time augmentation (flips + 90-degree rotations)

Dataset format, CSV columns, default data root, and model architecture are
unchanged from train_unet_baseline.py, so it remains compatible with both the
standard strict splits and the strict PNG-filtered splits.

Example:
    python scripts/train_unet_experiment.py \
        ...
        --loss bce_dice --save-best-by val_dice --augment
"""
...
def print_basic_help() -> None:
    ...
    print(
        """usage: train_unet_experiment.py [options]

Train an experimental SAR-only binary U-Net with selectable loss,
checkpoint-selection metric, and optional augmentation.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-name", default="unet_experiment_strict_fp1")
