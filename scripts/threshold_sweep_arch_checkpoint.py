#!/usr/bin/env python3
"""Threshold sweep for a single arch-aware checkpoint.

Same procedure as threshold_sweep_single_checkpoint.py, but builds the model
via train_unet_arch_experiment.build_model(arch, base_channels) using the
arch/base_channels recorded in the checkpoint metadata, so it can load
checkpoints trained with --arch unet or --arch attention_unet.

Given one checkpoint, one validation CSV, and one test CSV:
  1. Load the checkpoint and read its arch/base_channels metadata.
...
Reuses FloodTileDataset/build_model from train_unet_arch_experiment.py and the
pixel-metric definitions from threshold_sweep_baseline.py, so results are
directly comparable with the other sweep scripts.

Usage:
    python scripts/threshold_sweep_arch_checkpoint.py \
        --checkpoint models/attention_unet_fp1_best.pt \
        ...
        --output-csv outputs/threshold_sweep_arch_checkpoint/attention_unet_fp1.csv \
        --device cuda
"""
...
def print_basic_help() -> None:
    ...
    print(
        """usage: threshold_sweep_arch_checkpoint.py [options]

Sweep thresholds on a validation split for one arch-aware checkpoint (as
produced by train_unet_arch_experiment.py), select the threshold that
maximizes validation Dice, then evaluate once on the test split at that
threshold.
...
The checkpoint must contain "arch", "base_channels", and "model_state_dict"
(as saved by train_unet_arch_experiment.py).
...
import train_unet_arch_experiment as _tae  # noqa: E402  (reuse FloodTileDataset / build_model)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Threshold sweep for a single arch-aware checkpoint."
    )
...
def main() -> None:
    ...
    model = _tae.build_model(arch, base_channels).to(device)
    ...
    # ------------------------------------------------------------------
    val_dataset = _tae.FloodTileDataset(args.val_csv, args.data_root, augment=False)
    ...
    # ------------------------------------------------------------------
    test_dataset = _tae.FloodTileDataset(args.test_csv, args.data_root, augment=False)
